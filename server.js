    ...
    try {
        ...
        for (let i = 0; i < arrLinks.length; i++) {
            ...
            try {
                ...
                axiosData = {imgUrl: "http://error.com/", img_x: 1, img_y: 1}
            }
...
/* app.post('/api/get-image-ids/', bodyParser.json(), async (req, res, next) => {
    ...
    try {
        ...
        for (let i = 0; i < arrLinks.length; i++) {
            const each = arrLinks[i];
            const axiosData = await axios.get(arrLinks[i]);
            let imgUrl;
            try {
                imgUrl = axiosData.data['images']['medium']['url'];
            } catch (error) {
...
app.get('/api/getCollection/*', async (req, res, next) => {
    try {
        ...
        // fs.writeFileSync('./_output.xml', axiosData.data, 'utf-8');
